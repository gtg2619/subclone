...
async function saveBlob(url, token, path){
    return getBlob(url, token).then(async (blob)=>{
...
async function customize(path, customPath){
    if(customPath){
        let splited = path.split('/');
        splited[0] = customPath;
        return splited.join('/');
    } else {
        return path;
    }
...
async function clone(target, opt){
    saveDir = await customize(opt.path || target.directory.slice(target.directory.lastIndexOf('/')+1), opt.customPath);
    ...
    await getJSON(url, opt).then(async (items)=>{
        items.forEach(async (item) => {
            let childOpt = opt;
            childOpt.path = await customize(item.path, opt.customPath);
